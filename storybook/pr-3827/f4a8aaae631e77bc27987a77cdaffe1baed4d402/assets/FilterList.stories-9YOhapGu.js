...
/>`}}},render:({objectType:t,objectSet:n,...i})=>{const r=o.useMemo(()=>[R,A],[]);return e.jsx("div",{style:u,children:e.jsx(d,{objectType:a,filterDefinitions:r,...i})})}},D={parameters:{docs:{description:{story:"Repro for the integer-typed `NUMBER_RANGE` rounding fix. `employeeNumber` is an `integer` property, so histogram bucket boundaries — `(maxValue - minValue) / 20` — are usually fractional. Click any bar in the histogram. The Min/Max boxes must show whole integers (e.g. `657495073`), not fractional values (e.g. `657495073.4`)."},source:{code:`<FilterList
  objectType={Employee}
  filterDefinitions={[
    {
      type: "PROPERTY",
...
<FilterList
  ...
  filterClause={filterClause}
  onFilterClauseChanged={setFilterClause}
/>`}}},render:t=>e.jsx(yn,{...t})};function Cn(t){const[n,i]=o.useState(void 0),[r,l]=o.useState(E),s=t.onFilterRemoved,p=o.useCallback(h=>{l(F=>F.filter(le=>"key"in le?le.key!==h:!0)),s==null||s(h)},[s]),c=t.onReset,f=o.useCallback(()=>{l(E),c==null||c()},[c]),m=t.onFilterClauseChanged,y=o.useCallback(h=>{i(h),m==null||m(h)},[m]);return e.jsxs("div",{style:Ut,children:[e.jsx("div",{style:Xt,children:e.jsx(d,{objectType:a,filterDefinitions:r,titleIcon:Kt,...t,onReset:f,onFilterRemoved:p,filterClause:n,onFilterClauseChanged:y})}),e.jsx("div",{style:S,children:e.jsx(Bt,{objectType:a,filter:n})})]})}const q={name:"Full Featured",args:{title:"Employee Filters",showResetButton:!0,showActiveFilterCount:!0,enableSorting:!0},parameters:{docs:{description:{story:"Demonstrates all filter list features together: collapse, reset, active count, sorting, removable filters, per-filter search, exclude toggle, and controlled where clause driving an ObjectTable."},source:{code:`// All features combined: collapse, sort, search, exclude, remove, reset
...
<FilterList
  ...
  filterClause={filterClause}
  onFilterClauseChanged={setFilterClause}
/>`}}},render:t=>e.jsx(Ln,{...t})},An={type:"PROPERTY",id:"department-multi",key:"department",label:"Department",filterComponent:"MULTI_SELECT",defaultFilterState:{type:"SELECT",selectedValues:[]}},Fn={type:"PROPERTY",id:"department-single",key:"department",label:"Department (single)",filterComponent:"SINGLE_SELECT",defaultFilterState:{type:"SELECT",selectedValues:[]}},Mn=[R,An,Fn,ie],Q={name:"No value rendering",parameters:{docs:{description:{story:"Empty/null filter values render via the canonical `<NoValueLabel />` component — italic, muted, with the literal text 'No value' — across listogram buckets, single-select dropdown options, multi-select dropdown options, and multi-select chips. The mock dataset includes one Employee with `department: \"\"` so the No value row is visible in the listogram."},source:{code:`// Empty/null values render via <NoValueLabel /> across every filter type.
<FilterList
  objectType={Employee}
  filterDefinitions={[
    {
