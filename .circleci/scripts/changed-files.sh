...
#
# On a topic branch the base is the merge-base with $BASE_BRANCH (default:
# main). On $BASE_BRANCH itself it is the previous commit, which mirrors what
# `github.event.before` gives the GitHub Actions workflows.
#
# Exits 2 (printing nothing to stdout) when the base commit cannot be resolved.
...
if [ -z "$base" ]; then
  echo "Could not resolve a base commit to diff against." >&2
  exit 2
fi
