...
Calendar primitives (`DateCalendar`, `DateRangeCalendar`, `LazyDateCalendar`, `LazyDateRangeCalendar`, `calendarShared`, `DatePickerCommon.module.css`) move from `action-form/fields/` to a shared `shared/calendar/` location and are re-exported from `shared/calendar/index.ts` so both `action-form` and `filter-list` import from one place. `formatDateForDisplay` gains an optional `locale` parameter for callers that want to pin a specific format across viewers; the default behavior (browser locale) is unchanged.
