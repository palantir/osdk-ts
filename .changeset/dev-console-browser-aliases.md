...
Add `@osdk/aliases`, extracting the alias runtime so consumers that are not Functions can read aliases without depending on `@osdk/functions`. The synchronous `Aliases` namespace exported by `@osdk/functions` remains behaviorally unchanged, but is deprecated ahead of a future asynchronous API.
