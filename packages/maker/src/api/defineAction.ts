...
// The parameter is a list when its `required` config carries a `listLength`.
function isListParameter(required: unknown): boolean {
  return (
    typeof required === "object" && required != null && "listLength" in required
  );
}

// `objectReference` / `objectReferenceList` parameter type for the target
// object, singular or list depending on `isList`.
function objectReferenceParamType(
  def: ActionTypeUserDefinition | InterfaceActionTypeUserDefinition,
  isList: boolean
): ActionParameter["type"] {
  const objectTypeId = def.objectType!.apiName;
  if (isList) {
    return {
  ...
  return { type: "objectReference", objectReference: { objectTypeId } };
}

// `interfaceReference` / `interfaceReferenceList` parameter type for the target
// interface, singular or list depending on `isList`.
function interfaceReferenceParamType(
  def: InterfaceActionTypeUserDefinition,
  isList: boolean
): ActionParameter["type"] {
  const interfaceTypeRid = def.interfaceType.apiName;
  if (isList) {
    return {
      type: "interfaceReferenceList",
      interfaceReferenceList: { interfaceTypeRid },
    };
  }
  return {
    type: "interfaceReference",
    interfaceReference: { interfaceTypeRid },
  };
}

function buildModifyObjectParam(
  def: ActionTypeUserDefinition | InterfaceActionTypeUserDefinition
): ActionParameter {
  const config = def.parameterConfiguration?.[MODIFY_OBJECT_PARAMETER];
  return {
    id: MODIFY_OBJECT_PARAMETER,
    displayName: config?.displayName ?? "Modify object",
    type: objectReferenceParamType(def, isListParameter(config?.required)),
    validation: {
      ...config,
...
function buildCreateOrModifyObjectParam(
  def: ActionTypeUserDefinition | InterfaceActionTypeUserDefinition
): ActionParameter {
  const config =
    def.parameterConfiguration?.[CREATE_OR_MODIFY_OBJECT_PARAMETER];
  return {
    id: CREATE_OR_MODIFY_OBJECT_PARAMETER,
    displayName: config?.displayName ?? "Create or modify object",
    type: {
      type: "objectReference",
      objectReference: {
        objectTypeId: def.objectType!.apiName,
        maybeCreateObjectOption: !("primaryKeyOption" in def)
          ? { type: "autoGenerated", autoGenerated: {} }
...
function buildDeleteObjectParam(
  def: ActionTypeUserDefinition | InterfaceActionTypeUserDefinition
): ActionParameter {
  const config = def.parameterConfiguration?.[DELETE_OBJECT_PARAMETER];
  return {
    id: DELETE_OBJECT_PARAMETER,
    displayName: config?.displayName ?? "Delete object",
    type: objectReferenceParamType(def, isListParameter(config?.required)),
    validation: {
      ...config,
      allowedValues: { type: "objectQuery" },
...
function buildCreateInterfaceObjectParam(
  def: InterfaceActionTypeUserDefinition
): ActionParameter {
  const config =
    def.parameterConfiguration?.[CREATE_INTERFACE_OBJECT_PARAMETER];
  return {
    id: CREATE_INTERFACE_OBJECT_PARAMETER,
    displayName: config?.displayName ?? "Object type to create",
    type: isListParameter(config?.required)
      ? {
          type: "objectReferenceList",
...
function buildModifyInterfaceObjectParam(
  def: InterfaceActionTypeUserDefinition
): ActionParameter {
  const config =
    def.parameterConfiguration?.[MODIFY_INTERFACE_OBJECT_PARAMETER];
  return {
    id: MODIFY_INTERFACE_OBJECT_PARAMETER,
    displayName: config?.displayName ?? "Object type to modify",
    type: interfaceReferenceParamType(def, isListParameter(config?.required)),
    validation: {
      ...config,
      required: config?.required ?? true,
      allowedValues:
...
function buildDeleteInterfaceObjectParam(
  def: InterfaceActionTypeUserDefinition
): ActionParameter {
  const config = def.parameterConfiguration?.[DELETE_OBJECT_PARAMETER];
  return {
        ...
        [
          ...
          switch (ref.type) {
            ...
            case "createdInterfaceObjectReferenceByUniqueIdentifier":
              parameterIds.add(
                ref.createdInterfaceObjectReferenceByUniqueIdentifier.objectType
              );
              break;
