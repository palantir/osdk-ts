...
/**
 * Checks the ontology's interface schema migrations against the persisted lockfile and brings
 * the lockfile up to date when `writeLocks` is set.
 *
 * Throws if the source contains a change that would be rejected at installation-time, or if the
 * lockfile is out of date and `writeLocks` was not passed.
 */
export async function reconcileOntologySchemaLockfile(
  ...
  if (!writeLocks) {
    const stale = staleInterfaces(persisted, expected);
    if (stale.length > 0) {
      throw new Error(formatStaleness(stale, lockfilePath));
    }
...
async function reconcileMissingLockfile(
  ...
  if (!writeLocks) {
    const count = lockedInterfaceCount(expected);
    throw new Error(
      `${count} ${count === 1 ? "interface is" : "interfaces are"} opted into schema ` +
...
function formatStaleness(stale: string[], lockfilePath: string): string {
  return (
    `Ontology schema lockfile ${lockfilePath} is out of date. These interfaces have changed since ` +
    `it was written:\n${stale.map((apiName) => `  ${apiName}`).join("\n")}\n\n` +
    `The changes are backwards-compatible, but the lockfile must record them before the ontology ` +
    `can be published. Run maker again with --write-locks.`
  );
}
