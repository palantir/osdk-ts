...
// Successive states an interface type migration would walk through
...
export function useLockfileHarness(): LockfileHarness {
  ...
  beforeEach(async () => {
    // Reconcile warns about anything it noticed but did not reject, which for most tests here is
    // incidental. The tests that are about a warning spy on `warn` themselves.
  ...
  return {
    ...
    // The header describes the file rather than the schema state, so drop it the way the parser
    // does and hand back something that really is an OntologySchemaLockfile.
    readLockfile: async () => {
      const { [LOCKFILE_HEADER_KEY]: _header, ...lockfile } = JSON.parse(
