...
describe("Generate Package Command", () => {
  ...
  it.each(["@test-app2", "@test-app2-beta"])("has a public scripts export for %s", async scope => {
    const generatedPath = path.join(path.dirname(fileURLToPath(import.meta.url)), "..", "build", "codegen", scope, "osdk");
    ...
    const contents = await fs.readFile(path.join(generatedPath, "esm", "index.js"), "utf-8");
    expect(contents).not.toContain("Object.defineProperty(exports,");
  });
});
//# sourceMappingURL=GeneratePackageCommand.test.js.map