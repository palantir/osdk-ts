...
- **Public re-export barrel: `src/public/internal.ts`** (exposed as the `@osdk/react-components/internal` subpath). Enumerates primitives that are intentionally re-exported (`ActionButton`, `Dialog`, `SkeletonBar`, `Tooltip`, etc.). Anything in `src/base-components/` not re-exported through `internal.ts` stays internal-only
