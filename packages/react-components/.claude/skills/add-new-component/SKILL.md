...
4. **Build the Base component first** (CONTRIBUTING.md "Component Architecture", README.md "Component Architecture", CLAUDE.md "OSDK Component Architecture"):
   ...
   - **Prefer Blueprint icons (`@blueprintjs/icons`) for iconography.** When you need an icon, use Blueprint's icon set rather than inlining SVGs, importing from another icon library, or hand-rolling glyphs. Blueprint icons match the package's visual language and are already a workspace dependency. Verify the icon name exists at the chosen export path before committing — see step 3 of the verification loop for the runtime check that catches typos.
