...
/** Fields carried by every active filter, regardless of kind. */
interface ActiveFilterFields<S extends FilterState> {
...
/** Fields carried by an active clause-contributing filter. */
interface ActiveClauseFilterFields<
  Q extends ObjectTypeDefinition,
  S extends FilterState,
> extends ActiveFilterFields<S> {
  /** The clause this filter alone contributes. */
  filterClause: WhereClause<Q>;
}

/** Fields carried by every active link-traversing filter (`HAS_LINK`, `LINKED_PROPERTY`). */
interface ActiveLinkedFilterFields<
  S extends FilterState,
  L extends string,
> extends ActiveFilterFields<S> {
  linkName: L;
...
export type ActiveHasLinkFilter<Q extends ObjectTypeDefinition> = {
  [L in LinkNames<Q>]: ActiveLinkedFilterFields<HasLinkFilterState, L> & {
    kind: "HAS_LINK";
  };
}[LinkNames<Q>];
...
export type ActiveLinkedPropertyFilter<Q extends ObjectTypeDefinition> = {
  [L in LinkNames<Q>]: ActiveLinkedFilterFields<
    LinkedPropertyFilterState,
    L
  > & {
    ...
    linkedFilterClause: WhereClause<LinkedType<Q, L>>;
  };
}[LinkNames<Q>];
