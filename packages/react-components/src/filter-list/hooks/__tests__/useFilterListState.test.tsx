...
describe("useFilterListState", () => {
  ...
  describe("hasChangesFromInitial", () => {
    ...
    it(
      "reset() restores the original snapshot even if filterDefinitions change at runtime",
      () => {
        const initialDef = createPropertyFilterDef(
          "name",
          "LISTOGRAM",
          createExactMatchState(["John"]),
        );
        const { result, rerender } = renderHook(
          ...
          { initialProps: [initialDef] },
        );
        const filterKey = getFilterKey(initialDef);
