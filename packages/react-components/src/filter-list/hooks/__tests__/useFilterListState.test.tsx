...
describe("useFilterListState", () => {
  ...
  it("updates filter state via setFilterState", () => {
    const props = createProps({
      filterDefinitions: [
        createPropertyFilterDef("name", "CHECKBOX_LIST", createCheckboxListState([])),
      ],
    });
    const { result } = renderHook(() => useFilterListState(props));
    act(() => {
      result.current.setFilterState("name:0", createCheckboxListState(["selected"]));
    });
  ...
  it("builds whereClause from filter states", () => {
    ...
    const { result } = renderHook(() => useFilterListState(props));
    act(() => {
      result.current.setFilterState("name:0", createCheckboxListState(["John"]));
    });
