...
describe("getActiveFilters", () => {
  ...
  it("tags a property filter with its own filterClause", () => {
  ...
  it("gives each filter only the clause it contributes", () => {
    ...
    expect(
      active.map((f) => ("filterClause" in f ? f.filterClause : undefined)),
    ).toEqual([{ name: "John" }, { id: "abc" }]);
  });
  ...
  it("tags a HAS_LINK filter with its link, and no filterClause", () => {
    ...
    expect(active[0]).not.toHaveProperty("filterClause");
  });
