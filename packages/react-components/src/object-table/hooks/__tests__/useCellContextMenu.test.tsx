...
describe("useCellContextMenu", () => {
  ...
  describe("position calculation with scroll", () => {
    it("calculates position correctly with no scroll", () => {
    ...
    it("calculates position correctly with scroll values", () => {
  ...
  describe("integration scenarios", () => {
    ...
    it("updates tdRef between renders", () => {
      const tdRef = { current: mockElement };
      const { result, rerender } = renderHook(
        ({ ref }) => useCellContextMenu({ tdRef: ref }),
        { initialProps: { ref: tdRef } },
      );
      ...
      // Open with first element
      act(() => {
        result.current.handleOpenContextMenu(mockEvent);
      });

      expect(result.current.popoverPosition?.left).toBe(110);

      // Create new element with different position
      const newElement = document.createElement("td");
      vi.spyOn(newElement, "getBoundingClientRect").mockReturnValue({
        left: 300,
        top: 400,
        bottom: 440,
        width: 100,
      ...
      // Open with new element
      act(() => {
        result.current.handleOpenContextMenu(mockEvent);
      });

      expect(result.current.popoverPosition?.left).toBe(310); // 300 + 10
    });
  });
});
