...
This table is generated from the `ObjectTableProps` interface in [`ObjectTableApi.ts`](https://github.com/palantir/osdk-ts/blob/main/packages/react-components/src/object-table/ObjectTableApi.ts). See [Column Definitions](#column-definitions) below for the shape of the `columnDefinitions` prop.
...
Type parameters: `Q extends ObjectOrInterfaceDefinition`, `RDPs extends Record<string, SimplePropertyDef> = {}`, `FunctionColumns extends Record<string, QueryDefinition<{}>> = Record<string, never>`
...
| `onLoadedObjectsChanged`    | `(change: LoadedObjectsChange<Q, RDPs>) => void`                                                                                         | Called when the set of loaded rows changes — as pages are fetched, when streamed updates arrive, and after a refetch — with a `LoadedObjectsChange` payload.                                                                                                                                                                                                                                                                                                                   |
...
| `resizable`       | `boolean`                                                                                                                                                                                                      |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `orderable`       | `boolean`                                                                                                                                                                                                      |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `renderCell`      | `(object: Osdk.Instance<Q, "$allBaseProperties", PropertyKeys<Q>, RDPs>, locator: ColumnDefinitionLocator<Q, RDPs, FunctionColumns>, value: unknown) => React.ReactNode`                                       | Custom renderer for the cell value.<br /><br />This changes only how the cell's value is displayed; the value itself is unchanged. To change the value, use `getCellValue` (and declare its `cellValueType`) — `renderCell` then receives that value as its third argument. The two can be combined.<br /><br />Interaction with `editable` columns: - When `editMode: "manual"` (default), `renderCell` is used while the table is read-only (Edit Table button visible) and the editable cell takes over once the user enters edit mode. - When `editMode: "always"`, the editable cell always wins on `editable: true` columns and `renderCell` is ignored — `editMode: "always"` opts the column into a permanently-editable surface, leaving no read-only state for `renderCell` to render. Use `editMode: "manual"` if you need a custom display alongside editing.<br /><br />The editor only takes over on cells that are actually editable. When `editable` is a predicate, rows it rejects stay read-only and keep rendering `renderCell` under either `editMode` — including `"always"`, where the table is permanently in edit mode. |
...
| `locator`         | `\| PropertyColumnLocator<Q> \| RdpColumnLocator<Q, RDPs> \| CustomColumnLocator`                                                                                                                              | **Required.** What the column shows — a property, a linked object property (derived property), or a custom column — and that locator's configuration.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `cellValueType`   | `BaseWirePropertyTypes`                                                                                                                                                                                        | The cell value's data type. Constrains what `getCellValue` may return, and selects the editor for an `editable` column.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `getCellValue`    | `(object: Osdk.Instance<Q, "$allBaseProperties", PropertyKeys<Q>, RDPs>, locator: ColumnDefinitionLocator<Q, RDPs, FunctionColumns>) => PropertyValueWireToClient[BaseWirePropertyTypes] \| null \| undefined` | Derives the cell's value from the row's object. Without it the column reads the property named by `locator.id`.<br /><br />Use this to change what the cell's value _is_ — what it returns is the value the table renders, edits, and passes on to `renderCell`. Use `renderCell` to change only how that value is displayed. The two can be combined.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
### Observing Loaded Objects

`onLoadedObjectsChanged` reports which rows the table currently holds. It fires once the first page has loaded and again each time the loaded rows or the total count change — as further pages are fetched, when streamed updates arrive, after a refetch, and as function-backed column values fill in. It's a listener only; the table keeps loading whether or not you pass it.
...
The payload is a `LoadedObjectsChange<Q, RDPs>`:

| Field           | Type                                                              | Description                                                                                                                                     |
| --------------- | ----------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| `loadedObjects` | `Osdk.Instance<Q, "$allBaseProperties", PropertyKeys<Q>, RDPs>[]` | Rows loaded so far, in display order. Rows the user hasn't scrolled to yet are absent. Each row also carries its function-backed column values. |
| `totalCount`    | `string \| undefined`                                             | Total objects matching the object set, or `undefined` when the API didn't report one. A string, matching `getSnapshot()`'s `totalCount`.        |

Compare `loadedObjects.length` against `totalCount` to tell whether more rows remain. To get **every** matching row rather than the loaded ones, use [`getSnapshot()`](#exporting-data) instead — it pages through the whole object set.

Memoize the handler (`useCallback`) if it closes over state, as you would any other prop.
...
| `useLoadedObjectsChanged`                                      | Fires a callback when the loaded rows or the total count change                            |
