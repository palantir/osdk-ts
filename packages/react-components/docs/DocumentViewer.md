...
| `tiffRendererProps` | `Partial<Omit<BaseTiffViewerProps, "content" \| "className">>` | **Deprecated** — Use `tiffViewerProps` instead. Alias for `tiffViewerProps`, which takes precedence when set to a non-nullish value. Defaults to `undefined`.                 |
