...
| `tiffRendererProps`      | `Partial<Omit<BaseTiffViewerProps, "content" \| "className">>`            | **Deprecated** — Use `tiffViewerProps` instead. Alias for `tiffViewerProps`, which takes precedence if both are set. Defaults to `undefined`.                                 |
...
| `markdownRendererProps`  | `Partial<Omit<BaseMarkdownViewerProps, "content" \| "className">>`        | **Deprecated** — Use `markdownViewerProps` instead. Alias for `markdownViewerProps`, which takes precedence if both are set. Defaults to `undefined`.                         |
