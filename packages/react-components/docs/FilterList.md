...
- **Persist filter state with `initialFilterStates`** -- read changes out via `onFilterStateChanged`, key them with the exported `getFilterKey(definition)`, and pass them back through `initialFilterStates` to rehydrate after navigation. `serializeFilterStates` / `deserializeFilterStates` handle the `Date` round-trip for you
