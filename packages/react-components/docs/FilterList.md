...
**Auto-filtering** — set `reverseLinkName`. FilterList composes `pivotTo(linkName).where(...).pivotTo(reverseLinkName)` and emits the filtered set via `onEffectiveObjectSet`:
...
**UI-only** — omit `reverseLinkName`. The filter still renders and fires `onFilterStateChanged`; downstream filtering is up to the consumer:

```typescript
{
  type: "LINKED_PROPERTY",
  linkName: "manager",
  // no reverseLinkName — FilterList won't filter objectSet on this filter
  linkedPropertyKey: "fullName",
  filterComponent: "MULTI_SELECT",
}
```
