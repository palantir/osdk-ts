...
| `onFilterAdded`            | `(filterKey: FilterKey<Q>) => void`                                            | Called after a filter is shown from the built-in "Add filter" popover.<br /><br />Uncontrolled mode only — controlled mode renders no built-in popover, so nothing fires this. For the resulting visibility and order, use `onFilterVisibilityChange`, which fires in both modes.                                                                                                                                                                                                              |
