...
| `onFilterChanged`          | `(change: FilterChangeEvent<Q>) => void`                                       | Called whenever filter state changes — a filter set, a filter cleared, or a reset — with the resulting clause, filtered `ObjectSet` and active filters in a single payload.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `filterStates`             | `Map<string, FilterState>`                                                     | Controlled mode. Filter states keyed by `getFilterKey`, replacing whatever the filters currently hold — on mount, and again every time a different map is passed. Read changes back out from `onFilterChanged`'s `filterStates` and pass them in here to restore filters later, e.g. per tab or per saved view. Also the state the reset button restores to.<br /><br />Editing a filter still renders immediately; the map only has to come back for a change to survive a later push. Takes precedence over `defaultFilterStates` and over each definition's own `defaultFilterState`, which are ignored entirely when this is supplied. Defaults to undefined (uncontrolled; filters seed from `defaultFilterStates` and their definitions) |
| `defaultFilterStates`      | `Map<string, FilterState>`                                                     | Uncontrolled mode. Seeds filter states from external storage, keyed by `getFilterKey`. Applied over the per-definition `defaultFilterState` seeds on the first render only, and FilterList owns the states from then on. Also the state the reset button restores to. Use `onFilterChanged` to persist changes back out.<br /><br />Ignored when `filterStates` is supplied. Defaults to `undefined (filters seed from their definitions alone)`.                                                                                                                                                                                                                                                                                              |
...
<FilterList
  ...
  onFilterChanged={(change: FilterChangeEvent<typeof Employee>) => {
    ...
    // change.filterStates      — every filter's state, keyed by getFilterKey
    ...
    switch (change.event) {
      ...
      case "RESET":
      case "REPLACE":
        persistAll(change.filterStates);
        break;
    }
  }}
/>;
...
| `"RESET"`   | —                       | The reset button restored the last given state            |
| `"REPLACE"` | —                       | A new `filterStates` prop replaced every filter's state   |
...
- **Persist filter state with `defaultFilterStates`** -- read changes out via `onFilterChanged`'s `filterStates`, keyed by the exported `getFilterKey(definition)`, and pass them back through `defaultFilterStates` to rehydrate after navigation. `serializeFilterStates` / `deserializeFilterStates` handle the `Date` round-trip for you
- **Switch between saved views with `filterStates`** -- pass the states in instead of seeding them when the same FilterList has to show a different set of filters over its lifetime, e.g. one per tab or saved view. Every new map replaces the filters and becomes the reset baseline, and reports an `onFilterChanged` with `event: "REPLACE"` carrying the clause and filtered `ObjectSet` it produces, so a table driven off the callback follows along. Editing a filter still renders straight away, so typing stays responsive whether or not the map comes back
