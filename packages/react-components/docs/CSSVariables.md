...
| `--osdk-filter-item-header-action-bg-active`     | `var(--osdk-custom-color-gray-2)`           | Action button active (mouse-down) background   |
...
| `--osdk-filter-item-exclude-dropdown-trigger-indicator-active`   | `inset 0 calc(var(--osdk-surface-border-width) * -4) 0 var(--osdk-filter-item-exclude-dropdown-trigger-color)` | Dropdown trigger inset underline shown in selected (Excluding) state |
