 ...
 */
export async function publishDevModeSettings(server, widgetIdToOverrides, res) {
  try {
    if (process.env.FOUNDRY_TOKEN == null) {
      throw new Error("FOUNDRY_TOKEN environment variable not found, unable to start dev mode.");
    }
    ...
    const settingsResponse = await setWidgetSetSettings(foundryUrl, widgetSetRid, widgetIdToOverrides);
    if (settingsResponse.status !== 200) {
      ...
      throw new Error(`Unable to set widget settings in Foundry: ${settingsResponse.statusText}`);
    }
    ...
    if (enableResponse.status !== 200) {
      ...
      server.config.logger.warn(responseContent);
      throw new Error(`Unable to enable dev mode in Foundry: ${enableResponse.statusText}`);
    }
    res.setHeader("Content-Type", "application/json");
    res.end(JSON.stringify({
      status: "success",
      redirectUrl: `${foundryUrl}/workspace/custom-widgets/preview/${widgetSetRid}`
    }));
  } catch (error) {
...
//# sourceMappingURL=publishDevModeSettings.js.map