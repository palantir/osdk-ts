...
import { CONFIG_FILE_SUFFIX, ENTRYPOINTS_PATH, PALANTIR_PATH, SETUP_PATH, VITE_INJECTIONS_PATH } from "../common/constants.js";
...
export function FoundryWidgetDevPlugin() {
  ...
  return {
    ...
    configureServer(server) {
       ...
       */
      server.middlewares.use(`/${SETUP_PATH}`, sirv(path.resolve(DIR_DIST, "../../site"), {
        single: true,
        dev: true
      }));
       ...
       */
      server.middlewares.use(`/${ENTRYPOINTS_PATH}`, (_, res) => {
       ...
       */
      server.middlewares.use(`/${PALANTIR_PATH}/finish`, async (_, res) => {
        ...
        const widgetIdToOverrides = await getWidgetIdOverrideMap(server, codeEntrypoints, configFileToEntrypoint, configFiles, getLocalhostUrl(server));
       ...
       * work correctly.
       */
      server.middlewares.use(`/${VITE_INJECTIONS_PATH}`, async (_, res) => {
...
function getLocalhostUrl(server) {
  return `${server.config.server.https ? "https" : "http"}://localhost:${server.config.server.port}`;
}
