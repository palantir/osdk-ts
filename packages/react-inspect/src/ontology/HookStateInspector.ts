...
import { safeFiberOperation } from "../fiber/SafeFiberOperation.js";
import { traverseAllFibers, walkFiberTree } from "../fiber/traverseFiber.js";
import type { Fiber, FiberRoot, MemoizedState } from "../fiber/types.js";
...
// structural check (no @osdk/react import) so detection stays dependency-free:
// OSDK-connected = a consumed context value has observableClient + client
export function isOsdkContextConsumer(fiber: Fiber): boolean {
  return getContextValues(fiber).some(
    (value) =>
      value != null
      && typeof value === "object"
