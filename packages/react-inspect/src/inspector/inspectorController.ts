...
export function createInspectorController(
  ...
  const {
    ...
    eventHandlers = {},
    resolveTarget = nearestUserComponent,
  } = options;

  const ignoreSelector = `[${INSPECTOR_IGNORE_ATTRIBUTE}]`;
  ...
  function detectElementAtPosition(clientX: number, clientY: number): void {
    const element = getElementAtPosition(clientX, clientY, ignoreSelector);
    ...
    const componentFiber = resolveTarget(fiber);
    if (!componentFiber) {
      setState({
        hoveredElement: element,
        inspectedComponent: null,
