...
import { nearestOsdkComponent } from "../ontology/nearestOsdkComponent.js";
import { InspectorBanner } from "./components/InspectorBanner.js";
import { InspectorOverlay } from "./components/InspectorOverlay.js";
import { INSPECTOR_IGNORE_ATTRIBUTE } from "./constants.js";
import { createInspectorController } from "./inspectorController.js";
import { nearestUserComponent } from "./targetResolvers.js";
import type {
  InspectedComponent,
  InspectMode,
  InspectorBannerConfig,
  InspectorController,
...
export interface ClickToInspectOptions {
  ...
  onActivate?: () => void;
  onDeactivate?: () => void;
  // "ontology" selects only OSDK components; defaults to "generic" (any component)
  mode?: InspectMode;
  // overrides mode with a custom resolver
  resolveTarget?: TargetResolver;
  banner?: InspectorBannerConfig;
}
...
export class ClickToInspectSystem {
  ...
  activate(): void {
    ...
    this.inspectorController = createInspectorController({
      ...
      resolveTarget: this.resolveTargetStrategy(),
      eventHandlers: {
        onActivate: () => {
  ...
  private resolveTargetStrategy(): TargetResolver {
    if (this.options.resolveTarget) {
      return this.options.resolveTarget;
    }
    return this.options.mode === "ontology"
      ? nearestOsdkComponent
  ...
  private showOverlay(): void {
    ...
    if (!this.overlayContainer) {
      ...
      this.overlayContainer.id = "react-inspect-overlay";
      this.overlayContainer.setAttribute(INSPECTOR_IGNORE_ATTRIBUTE, "");
  ...
  private showBanner(): void {
    ...
    if (!this.bannerContainer) {
      ...
      this.bannerContainer.id = "react-inspect-banner";
      renderTarget.appendChild(this.bannerContainer);
      this.bannerRoot = createRoot(this.bannerContainer);
    }

    this.bannerRoot?.render(
      React.createElement(InspectorBanner, {
        visible: true,
        message: this.options.banner?.message,
        exitKeyLabel: this.options.banner?.exitKeyLabel,
        exitHintText: this.options.banner?.exitHintText,
      }),
    );
  }
