...
const INSTRUMENTATION_SOURCE = "react-inspect-devtools";
...
function installDevToolsHook(onActive?: () => void): ReactDevToolsGlobalHook {
  ...
  let hook: ReactDevToolsGlobalHook = {
    ...
    _instrumentationSource: INSTRUMENTATION_SOURCE,
    checkDCE,
...
function patchDevToolsHook(onActive?: () => void): void {
  ...
  try {
    ...
    if (hook._instrumentationSource === INSTRUMENTATION_SOURCE) {
    ...
    // Mark as patched
    hook._instrumentationSource = INSTRUMENTATION_SOURCE;
