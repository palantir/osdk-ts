...
export function mutateReleasePlan(releasePlan, releaseType) {
  for (const changeSet of releasePlan.changesets) {
    for (const release of changeSet.releases) {
      if (releaseType === "main" && release.type === "patch") {
        release.type = "minor";
      } else if (releaseType === "patch" && release.type !== "patch" && release.type !== "none") {
        throw new FailedWithUserMessage(`Releasing requires converting a ${release.type} to a patch, but that may not be safe.`);
      }
    }
  }
...
//# sourceMappingURL=mutateReleasePlan.js.map