...
export async function publishPackages(cwd, {
  tag,
  gitTag = true
}, config) {
  ...
  const r = await pMap(unpublishedPackagesInfo, async pkg => await publishSinglePackage(pkg, config.access, releaseTag), {
    concurrency: 4
  });

  // need to produce the output to save for future run of create tag/release
  await fs.writeFile(path.join(cwd, "pnpm-publish-summary.json"), JSON.stringify({
...
async function execPnpmPublish({
  ...
  const result = await execa("pnpm", ["publish", "--json", "--tag", "--dry-run", tag, "--no-git-checks"], {
    cwd
  });
  if (result.exitCode !== 0) {
    consola.error(`Failed to publish ${chalk.cyan(cwd)} with error: ${result.stderr}`);
    return false;
  }
  consola.success(`Published ${chalk.cyan(cwd)} with tag ${chalk.green(tag)}`);
  return true;
}
async function publishSinglePackage(pkg, access, tag) {
  const {
    name,
    version,
    publishConfig
  } = pkg.packageJson;
  consola.info(`Publishing ${chalk.cyan(`"${name}"`)} at ${chalk.green(`"${version}"`)}`);
  return {
    name,
    newVersion: version,
    published: await execPnpmPublish({
      cwd: pkg.dir,
      tag,
      access
    })
...
export async function packageVersionsOrEmptySet(name) {
  try {
...
async function getUnpublishedPackages(packages) {
  return pFilter(packages, async pkg => {
    ...
    if (versions.has(version)) {
      consola.info(`${name} is being published because our local version (${version}) has not been published on npm`);
      return true;
    } else {
      consola.warn(`${name} is not being published because version ${version} is already published on npm`);
      return false;
    }
  });
}
