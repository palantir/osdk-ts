...
// Nested oxlint config for @osdk/tool.release. It inherits the whole repo ruleset by
...
export default defineConfig({
  ...
  rules: {
    "no-extra-boolean-cast": "off", // `!!x` double negation; intentional and clear.
    "no-template-curly-in-string": "off", // A literal `${...}` inside a normal string (emitted as text); intentional.
    "no-throw-literal": "off", // Throwing a non-Error value; intentional here.
    "prefer-template": "off", // String concatenation vs template literals; stylistic churn.
    "import/no-cycle": "off", // An existing import cycle; breaking it is a refactor.
    ...
    "unicorn/prefer-optional-catch-binding": "off", // Omitting the unused catch binding; a source rewrite.
    "unicorn/prefer-set-has": "off", // `Array#includes` vs `Set#has` for membership; a source rewrite.
