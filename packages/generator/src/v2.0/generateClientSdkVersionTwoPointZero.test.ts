...
describe("generator", () => {
  ...
  it("throws an error if a namespace is provided that all top levels do not use", async () => {
    ...
    await expect(
      ...
      `[Error: Invariant failed: objectNames: Person,Todo]`,
    );
  });
  ...
  it("does not throw an error if a namespace is provided that all top levels use", async () => {
    ...
    expect(helper.getFiles()).toMatchInlineSnapshot(`
      ...
      ",
        "/foo/ontology/queries/getCount.ts": "import { QueryDefinition } from '@osdk/api';

      export const getCount = {
        apiName: 'foo.bar.getCount',
        type: 'query',
        version: '0',
        parameters: { completed: { nullable: false, type: 'boolean' } },
        output: { nullable: false, type: 'integer' },
      } satisfies QueryDefinition<'foo.bar.getCount', never>;
      ",
