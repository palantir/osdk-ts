...
var FD = O(uD);
function A$1(t, u2 = {}) {
  if (typeof t != "string" || t.length === 0 || (u2 = {
...
function sD() {
  ...
  for (const [u2, F] of Object.entries(r)) {
    for (const [e2, s] of Object.entries(F)) r[e2] = {
      open: `\x1B[${s[0]}m`,
      close: `\x1B[${s[1]}m`
    }, F[e2] = r[e2], t.set(s[0], s[1]);
    Object.defineProperty(r, u2, {
      value: F,
      enumerable: false
    });
  }
  return Object.defineProperty(r, "codes", {
    value: t,
    enumerable: false
  }), r.color.close = "\x1B[39m", r.bgColor.close = "\x1B[49m", r.color.ansi = L(), r.color.ansi256 = N(), r.color.ansi16m = I(), r.bgColor.ansi = L(m$1), r.bgColor.ansi256 = N(m$1), r.bgColor.ansi16m = I(m$1), Object.defineProperties(r, {
    rgbToAnsi256: {
      value: (u2, F, e2) => u2 === F && F === e2 ? u2 < 8 ? 16 : u2 > 248 ? 231 : Math.round((u2 - 8) / 247 * 24) + 232 : 16 + 36 * Math.round(u2 / 255 * 5) + 6 * Math.round(F / 255 * 5) + Math.round(e2 / 255 * 5),
      enumerable: false
    },
    hexToRgb: {
      value: (u2) => {
    ...
    hexToAnsi256: {
      value: (u2) => r.rgbToAnsi256(...r.hexToRgb(u2)),
      enumerable: false
    },
    ansi256ToAnsi: {
      value: (u2) => {
    ...
    rgbToAnsi: {
      value: (u2, F, e2) => r.ansi256ToAnsi(r.rgbToAnsi256(u2, F, e2)),
      enumerable: false
    },
    hexToAnsi: {
      value: (u2) => r.ansi256ToAnsi(r.hexToAnsi256(u2)),
      enumerable: false
    }
  }), r;
}
...
`)];
  for (const [E2, a2] of o.entries()) {
    if (e2 += a2, v.has(a2)) {
      const {
        groups: B2
      } = new RegExp(`(?:\\${W}(?<code>\\d+)m|\\${y$1}(?<uri>.*)${w})`).exec(o.slice(E2).join("")) || {
        groups: {}
      };
...
var c = {
  actions: new Set(aD),
  aliases: /* @__PURE__ */ new Map([["k", "up"], ["j", "down"], ["h", "left"], ["l", "right"], ["", "cancel"], ["escape", "cancel"]])
};
function k$1(t, u2) {
...
var x = class {
  ...
  prompt() {
    return new Promise((u2, F) => {
      ...
      }), f__default.default.emitKeypressEvents(this.input, this.rl), this.rl.prompt(), this.opts.initialValue !== undefined && this._track && this.rl.write(this.opts.initialValue), this.input.on("keypress", this.onKeypress), d(this.input, true), this.output.on("resize", this.render), this.render(), this.once("submit", () => {
...
`), d(this.input, false), this.rl?.close(), this.rl = undefined, this.emit(`${this.state}`, this.value), this.unsubscribe();
  ...
  restoreCursor() {
    const u2 = G$1(this._prevFrame, process.stdout.columns, {
      hard: true
    }).split(`
`).length - 1;
  ...
  render() {
    const u2 = G$1(this._render(this) ?? "", process.stdout.columns, {
      hard: true
    });
...
var wD = class extends x {
  constructor(u2) {
    super(u2, false), Y(this, "options"), Y(this, "cursor", 0), this.options = u2.options, this.value = [...u2.initialValues ?? []], this.cursor = Math.max(this.options.findIndex(({
      value: F
    }) => F === u2.cursorAt), 0), this.on("key", (F) => {
      F === "a" && this.toggleAll();
    }), this.on("cursor", (F) => {
      switch (F) {
        case "left":
...
var jD = class extends x {
  constructor(u2) {
    super(u2, false), q(this, "options"), q(this, "cursor", 0), this.options = u2.options, this.cursor = this.options.findIndex(({
      value: F
    }) => F === u2.initialValue), this.cursor === -1 && (this.cursor = 0), this.changeValue(), this.on("cursor", (F) => {
      switch (F) {
        case "left":
        case "up":
          this.cursor = this.cursor === 0 ? this.options.length - 1 : this.cursor - 1;
...
var k = (s) => {
  const {
    ...
    options: t,
    style: i
  } = s, r2 = s.maxItems ?? Number.POSITIVE_INFINITY, c2 = Math.max(process.stdout.rows - 4, 0), o = Math.min(c2, Math.max(r2, 5));
  let l = 0;
...
${e.gray(a)}`;
        default:
          return `${t}${e.cyan(a)} ${k({
            ...
            maxItems: s.maxItems,
            style: (i, r2) => n(i, r2 ? "active" : "inactive")
          }).join(`
${e.cyan(a)}  `)}
${e.cyan(m)}
`;
      ...
      switch (this.state) {
        case "submit":
          return `${t}${e.gray(a)} ${this.options.filter(({
            value: r2
          }) => this.value.includes(r2)).map((r2) => n(r2, "submitted")).join(e.dim(", ")) || e.dim("none")}`;
        case "cancel": {
          const r2 = this.options.filter(({
            value: c2
          }) => this.value.includes(c2)).map((c2) => n(c2, "cancelled")).join(e.dim(", "));
...
//# sourceMappingURL=prompt-IKRSF7JL.cjs.map
//# sourceMappingURL=prompt-IKRSF7JL.cjs.map