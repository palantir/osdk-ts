...
describe(resolveDependenciesFromFindUp, () => {
  describe("not provided a version", () => {
    it("errors when it does not find a version", async () => {
      ...
      }, dirname(fileURLToPath(import.meta.url)))).rejects.toThrow(/^Invariant failed: Could not find a dependency for not_a_real_dep_you_would_find in (.*?)$/);
    });
  });
  describe("uses version passed to it", () => {
    const expectedPrefix = "^";
    it.each(["1.2.3", "~1.2.3", "^1.2.3"])(`replaces "%s" with "${expectedPrefix}1.2.3"`, async version => {
    ...
    it.each(["workspace:1.2.3", "workspace:~1.2.3", "workspace:^1.2.3"])(`replaces "%s" with "${expectedPrefix}1.2.3"`, async version => {
      const result = await resolveDependenciesFromFindUp({
        foo: version
      }, dirname(fileURLToPath(import.meta.url)));
      expect(result).toEqual([{
    ...
    it.each(["workspace:*", "workspace:^", "workspace:~"])(`replaces "%s" with "workspace:${expectedPrefix}"`, async version => {
      const result = await resolveDependenciesFromFindUp({
        foo: version
      }, dirname(fileURLToPath(import.meta.url)));
      expect(result).toEqual([{
...
//# sourceMappingURL=resolveDependenciesFromFindUp.test.js.map