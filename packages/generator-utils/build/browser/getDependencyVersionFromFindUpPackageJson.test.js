...
describe(getDependencyVersionFromFindUpPackageJson, () => {
  ...
  describe("inferring cwd", () => {
    it.each([fakeDepName, fakePeerDepName, fakeDevDepName])("finds the tmp package.json and value for %s", async depName => {
      await expect(getDependencyVersionFromFindUpPackageJson(depName)).resolves.toBe(`${fakeDepVersion}-${depName}`);
    });
  });
  describe("not inferring cwd", () => {
    it.each([fakeDepName, fakePeerDepName, fakeDevDepName])("does not find the value for %s", async depName => {
      await expect(getDependencyVersionFromFindUpPackageJson(depName, {
        cwd: dirname(fileURLToPath(import.meta.url))
      })).rejects.toThrowError(/^Invariant failed: Could not find a dependency/);
    });
    it("does find our typescript version", async () => {
      await expect(getDependencyVersionFromFindUpPackageJson("typescript", {
        cwd: dirname(fileURLToPath(import.meta.url))
      })).resolves.toBeDefined();
    });
  });
});
//# sourceMappingURL=getDependencyVersionFromFindUpPackageJson.test.js.map