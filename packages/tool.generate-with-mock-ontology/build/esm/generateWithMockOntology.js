...
export async function generateWithMockOntology() {
  try {
    const dir = await fs.mkdtemp(path.join(tmpdir(), "osdk-e2e-foundry-sdk-generator-"));
    apiServer.listen();
    const testApp2Dir = path.join(dir, "@test-app2");
    await fs.rm(testApp2Dir, {
      recursive: true,
      force: true
    });
    ...
    await generatePackageCommand.handler({
      ...{
        ...
        objectTypes: ["Employee", "Office", "objectTypeWithAllPropertyTypes", "ObjectWithTimestampPrimaryKey", "equipment"],
        actionTypes: ["createOffice", "moveOffice", "createOfficeAndEmployee", "actionTakesObjectSet"],
        queryTypes: ["addOne", "incrementPersonAge", "returnsTimestamp", "returnsDate", "returnsObject", "twoDimensionalAggregationFunction", "threeDimensionalAggregationFunction"],
    ...
    await $({
      stdout: "inherit",
      stderr: "inherit"
    })`attw --pack ${path.join(testApp2Dir, "osdk")} --ignore-rules internal-resolution-error`;
    const finalOutDir = path.join(process.cwd(), "osdk");
    await fs.rm(path.join(finalOutDir, "@test-app2"), {
      recursive: true,
      force: true
    });
    await fs.cp(dir, finalOutDir, {
      recursive: true
    });
  } finally {
...
//# sourceMappingURL=generateWithMockOntology.js.map