...
describe("createPublicOauthClient", () => {
  it("should return the same processed options for both client creation methods", async () => {
    ...
    vi.spyOn(utilsModule, "processOptionsAndAssignDefaults").mockImplementation((...args) => {
    ...
    vi.spyOn(oauthModule, "createPublicOauthClient").mockImplementation((...args) => {
      const [_client_id, ...rest] = args;
      mockProcessOptionsAndAssignDefaults(...rest);
      return originalCreatePublicOauthClient(...args);
    });
    const authClient = createPublicOauthClient(FOUNDRY_CLIENT_ID, FOUNDRY_URL, REDIRECT_URI, true, undefined, undefined, undefined, fetch, undefined);
    expect(authClient).toBeDefined();
    expect(mockProcessOptionsAndAssignDefaults).toHaveBeenCalledTimes(1);
    const authClientWithOptions = createPublicOauthClient(FOUNDRY_CLIENT_ID, FOUNDRY_URL, REDIRECT_URI, {
      useHistory: true,
      fetchFn: fetch
    });
...
//# sourceMappingURL=createPublicOauthClient.test.js.map