...
describe(wireObjectTypeFullMetadataToSdkObjectMetadata, () => {
  ...
  it("preserves main-value and reducer metadata on struct arrays", () => {
    const result = wireObjectTypeFullMetadataToSdkObjectMetadata({
      implementsInterfaces: [],
      implementsInterfaces2: {},
      linkTypes: [],
          ...
          withMainValueAndReducers: {
            dataType: {
              ...
              subType: {
                ...
                mainValue: {
                  mainValueType: { type: "geopoint" },
                  fields: ["geopoint"],
                },
              },
              reducers: [{ direction: "DESCENDING_NULLS_LAST" }],
            },
            rid: "withMainValueAndReducersRid",
            typeClasses: [],
          },
        },
        rid: "rid",
    ...
    expect(result.properties.withMainValueAndReducers).toMatchObject({
      type: { geopoint: "geopoint", timestamp: "timestamp" },
      multiplicity: true,
      hasReducers: true,
      mainValue: { fields: ["geopoint"] },
    });
  });
