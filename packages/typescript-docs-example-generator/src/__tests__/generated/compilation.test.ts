...
describe("Generated Code Compilation", () => {
  ...
  it("should generate TypeScript code that compiles without errors", async () => {
    // Create temporary directory
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "test-compilation-"));
    const hierarchyPath = path.join(tempDir, "hierarchy.ts");
    ...
    try {
      ...
      async function findTsFiles(dir: string) {
        ...
        for (const entry of entries) {
          ...
          if (entry.isDirectory()) {
            ...
            tsFiles.push({ path: fullPath, content });
          }
        }
      }
      ...
      // Validate each file compiles successfully
      const compilationErrors: Array<{ file: string; errors: string[] }> = [];

      for (const { path: filePath, content } of tsFiles) {
        const relativePath = path.relative(tempDir, filePath);
        const fileName = path.basename(filePath);

        // Validate TypeScript compilation
        const result = validator.validateTypeScriptCode(content, fileName);

        if (!result.success) {
          compilationErrors.push({
            file: relativePath,
            errors: [`Validation failed: ${result.error.message}`],
          });
          continue;
        }
      ...
      console.log(`✅ All ${tsFiles.length} generated TypeScript files compile successfully`);
    } finally {
      // Clean up temporary directory
      await fs.rm(tempDir, { recursive: true, force: true });
    }
  ...
  it("should not skip valid templates containing '...'", async () => {
    // Create temporary directory
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "test-spread-"));
    const hierarchyPath = path.join(tempDir, "hierarchy.ts");

    try {
      // Run the generator for a version that should have spread syntax
      await generateExamples(tempDir, hierarchyPath, ["2.1.0"]);
      ...
      // Look for files containing spread operator
      let filesWithSpread = 0;
      ...
      // We expect at least some files to have valid '...' usage
      console.log(`Found ${filesWithSpread} files with '...' syntax out of ${tsFiles.length} total files`);
      
      // This assertion might need to be adjusted based on actual template content
      // For now, we just verify the generator runs without errors
      expect(tsFiles.length).toBeGreaterThan(0);
  ...
  it("should generate expected number of examples per version (accounting for valid skips)", async () => {
    ...
    const examplesPath = path.resolve(__dirname, "../../../../typescript-sdk-docs-examples/src/typescriptOsdkExamples.ts");
    const examplesContent = await fs.readFile(examplesPath, "utf-8");

    // Parse the generated examples file to extract the structure
    const examplesMatch = examplesContent.match(/export const TYPESCRIPT_OSDK_EXAMPLES = ({[\s\S]*?}) as const;/);
    ...
    for (const [version, versionData] of Object.entries(docsData.versions)) {
      if (!versionData || typeof versionData !== 'object' || !('snippets' in versionData)) {
        continue;
      }

      // Skip versions that aren't being generated (like 1.0.0)
      if (!generatedVersions.includes(version)) {
        console.log(`Skipping version ${version} - not in generated versions: ${generatedVersions.join(", ")}`);
        continue;
      }
      ...
      const actualCount = outputVersion?.examples ? Object.keys(outputVersion.examples).length : 0;

      console.log(`\nVersion ${version}:`);
      console.log(`  Input snippets: ${inputCount}`);
      console.log(`  Expected skips: ${expectedSkips}`);
      console.log(`  Templates with variations: ${variationGeneratingTemplates}`);
      ...
      if (missingSnippets.length > 0) {
        console.error(`\nCompletely missing templates in ${version}:`, missingSnippets);
      }
      ...
      if (variations.length > 0) {
        console.log(`\nGenerated variations (${variations.length}):`, variations.slice(0, 5), variations.length > 5 ? '...' : '');
      }

      // The key assertion: no templates should be completely missing
      if (missingSnippets.length > 0) {
        throw new Error(`Found ${missingSnippets.length} missing templates in version ${version}. Templates that should generate examples but don't: ${missingSnippets.join(', ')}`);
      }

      console.log(`✓ All expected templates generated output (base examples or variations)`);
