...
export async function runInterfacesTest3(): Promise<void> {
  console.log("start");
  // const metadata = await dsMtClient.fetchMetadata({
  //   type: "interface",
  //   apiName: "BagResolutionBag",
  // });
  // console.log(metadata);

  const objMetadata = await dsMtClient.fetchMetadata({
    type: "object",
    apiName: "Ocv4WorkerBags",
  });
  console.log(objMetadata.interfaceLinkMap);
  console.log(objMetadata.links);
  const myObj = await dsMtClient({
    "type": "object",
    "apiName": "Ocv4WorkerBags",
  }).fetchOne("brown - 7");
  console.log(
    myObj.$__EXPERIMENTAL__NOT_SUPPORTED_YET__metadata.ObjectMetadata,
  );
  console.log(myObj.$link);
