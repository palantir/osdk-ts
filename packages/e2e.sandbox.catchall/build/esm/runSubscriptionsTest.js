...
import { $Actions, MtaBus, OsdkTestInterface, OsdkTestObject } from "@osdk/e2e.generated.catchall";
...
function normalSubscription() {
  ...
  const subscription = client(OsdkTestObject).subscribe({
    onChange(object) {
      console.log("Object with primaryKey ", object.object.$primaryKey, " changed stringProperty to ", object.object.stringProperty);
      if (++counter >= 3) {
...
function interfaceSubscription() {
  client(OsdkTestInterface).subscribe({
    onChange(object) {
      console.log("Interface with primaryKey ", object.object.$primaryKey, " changed objectDescription to ", object.object.objectDescription);
    },
    async onSuccessfulSubscription() {
  ...
  const mtaBusSubscription = dsClient(MtaBus).subscribe({
    onChange(object) {
      if (object.object.positionId != null) {
        console.log("Bus with positionId ", object.object.vehicleId, " changed location to ", object.object.positionId.lastFetchedValue?.value);
      } else {
        console.log("Bus with vehicleId ", object.object.vehicleId, " changed nextStop to ", object.object.nextStopId);
      }
    },
    onError(err) {
      console.error("Error in subscription: ", err);
...
//# sourceMappingURL=runSubscriptionsTest.js.map