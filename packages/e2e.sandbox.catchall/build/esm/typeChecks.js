...
import { Employee, ObjectTypeWithAllPropertyTypes } from "@osdk/e2e.generated.catchall";
 ...
 * @param client
 */
export async function typeChecks(client) {
  // single link pivot types are correct
  {
    client(Employee).pivotTo("lead");
    expectType(true);
  }

  // multi link pivot types are correct
  {
    client(Employee).pivotTo("peeps");
    expectType(true);
  }

  // just a demo of aggregations
  {
    await client(ObjectTypeWithAllPropertyTypes).aggregate({
      $select: {
  ...
  // object $link examples
  {
    ...
    // lead is an employee
    await employee.$link.lead.fetchOne();
    expectType(true);
    ...
    await employee.$link.peeps.fetchPage({
    ...
    await employee.$link.peeps.fetchOne("peepPK", {
      $select: ["adUsername"]
    });
    expectType(true);
