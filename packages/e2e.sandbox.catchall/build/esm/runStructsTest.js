...
export async function runStructsTest() {
  const player = await dsClient(BgaoNflPlayer).fetchOne("50A409AB-C909-453A-A61A-31B51324C8E3");

  // Making sure things work when struct values are not set, like with this object
  console.log(player);
  console.log(player.address);
  console.log(player.address?.addressLine1);
  const airport = await dsClient(McAirportStruct).fetchOne("Ronald Reagan Washington National Airport");
  console.log(airport.airportStruct);
  console.log(airport.airportStruct?.geoHash);
  const airportFilteredShouldHaveData = await dsClient(McAirportStruct).where({
    $and: [{
      airportStruct: {
        code: {
          $startsWith: "D"
      ...
      airportName: {
        $containsAnyTerm: "Reagan"
      }
    }]
  }).fetchPage();
  const airportFilteredShouldNotHaveData = await dsClient(McAirportStruct).where({
    $and: [{
      airportStruct: {
        code: {
          $startsWith: "B"
...
void runStructsTest();
//# sourceMappingURL=runStructsTest.js.map