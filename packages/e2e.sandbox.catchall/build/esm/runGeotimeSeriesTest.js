...
import { FintrafficAis, GtfsTripTrackObject, RhemmingsObjectWithGtsrProperty2 } from "@osdk/e2e.generated.catchall";
...
export async function runGeotimeSeriesReferenceTests() {
  ...
  const secondResult = await dsClient(RhemmingsObjectWithGtsrProperty2).fetchOne("track-id");
  ...
  const resultWithNoData = await dsClient(GtfsTripTrackObject).fetchOne("wmata_35356020");
  const noDataPoint = await resultWithNoData.geotimeSeriesReferences?.getLatestValue();
  console.log("GTSR with no data should be undefined: ", noDataPoint);
}
void runGeotimeSeriesReferenceTests();
//# sourceMappingURL=runGeotimeSeriesTest.js.map