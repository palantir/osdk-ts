...
export async function runFoundrySdkClientVerificationTest(datasetRid, branchToCreate = "test") {
  const pageSize = 10;
  ...
  if (branchesResult.nextPageToken) {
    throw new Error(`You cannot run this test on a dataset with more than ${pageSize} branches`);
  }
  if (!branchesResult.data.find(b => b.name === "master")) {
    throw new Error(`You can not run this test as dataset ${datasetRid} does not have a master branch.`);
  }
  if (branchesResult.data.find(b => b.name === branchToCreate)) {
    throw new Error(`Expected that dataset ${datasetRid} would not have a branch called "${branchToCreate}". Aborting`);
  }
  ...
  try {
    ...
    if (err instanceof PalantirApiError) {
      ...
      logger.info({
        err
      }, "Expected an error and caught it");
    } else {
      throw err;
    }
  }
  ...
  logger.info(testBranch, "Created and deleted test branch, which means we handled the Promise<void> correctly");
}
//# sourceMappingURL=runFoundrySdkClientVerificationTest.js.map