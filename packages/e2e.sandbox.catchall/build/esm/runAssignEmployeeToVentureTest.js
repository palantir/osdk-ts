...
import { assignEmployee1, Employee, Person, Venture } from "@osdk/e2e.generated.catchall";
import { client as unstableClient } from "./client.js";
const client = unstableClient;
export async function runAssignEmployeeToVentureTest() {
  let didValidateOnce = false;
  await client(Employee).fetchOneWithErrors("hi", {
    // $select: ["adUsername"],
    $select: ["adUsername"]
  });
  ...
  client(Person);
  client(Person).fetchPage();
}
void runAssignEmployeeToVentureTest();
//# sourceMappingURL=runAssignEmployeeToVentureTest.js.map