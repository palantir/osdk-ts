...
export async function cli() {
  ...
  const templatesDir = await findUp("templates", {
    type: "directory"
  });
  ...
  const sourcePackageJson = JSON.parse(await fs.readFile(sourcePackageJsonPath, "utf-8"));
  let result = "export const files: Map<string, {type: 'base64', body: string} | {type: 'raw', body: string}> = new Map([\n";
  const processFiles = async function (dir, baseDir) {
    for (const filename of await fs.readdir(dir)) {
      ...
      } else if (extsToString.has(path.extname(destPath)) || path.basename(destPath) === ".gitignore") {
  ...
  result += `]);`;
  await fs.mkdir("src/generatedNoCheck", {
    recursive: true
  });
...
function safeRaw(q) {
  return `{ type: "raw",  body: \`${q.replace(/\\/g, "\\\\").replace(/`/g, "\\`").replace(/\$/g, "\\$")}\`}`;
}
