...
import type {
  GetGloballyScopedMonitorEventsResponse as _api_registry_GetGloballyScopedMonitorEventsResponse,
  PageToken as _api_registry_PageToken,
} from "../__components.js";

/**
 * This Monitor event is NOT user-scoped. Gets all events that are globally scoped (does not include metadata
 * events)
 * Throws:
 * - MonitorNotFound: if the monitor does not exist or the user does not have view permissions on it.
 */
export async function getGloballyScopedMonitorEvents(
  ctx: ConjureContext,
