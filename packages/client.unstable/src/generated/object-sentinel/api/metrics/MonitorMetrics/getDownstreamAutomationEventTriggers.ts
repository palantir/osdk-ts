...
/**
 * Finds automation events that were triggered by the provided automation event.
 */
export async function getDownstreamAutomationEventTriggers(
  ctx: ConjureContext,
  request: _api_metrics_GetAutomationEventDependenciesRequest,
): Promise<_api_metrics_GetAutomationTriggerDependenciesResponse> {
  return conjureFetch(
    ctx,
    `/monitorMetrics/downstreamAutomationEventTriggers`,
    "POST",
    request,
  );
}
