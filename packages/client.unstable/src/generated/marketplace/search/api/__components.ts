...
export interface IndexedBlockSet {
  ...
  outputRidPrefixes: Array<OutputRidPrefix>;
  outputRids: Array<OutputRid>;
  productName: string;
 ...
 *
 * Note: Versionless installations (registered but no job has run) are excluded from search indexing
 * because this type requires a blockSetVersionId. They will not appear in search results until their
 * first install job runs.
 */
...
export type OutputRid = string;
export type OutputRidPrefix = string;
