...
describe("applyShapeTransformations", () => {
  ...
  it("select op passes property value through unchanged", () => {
    const shape = createMockShape({
      name: { nullabilityOp: { type: "select" } },
    });
  ...
  it("require op returns data when non-null", () => {
    const shape = createMockShape({
  ...
  it("dropIfNull op passes through when non-null", () => {
    const shape = createMockShape({
      name: { nullabilityOp: { type: "dropIfNull" } },
    });
    const obj = createMockObject({ name: "Alice" });
  ...
  it("withDefault op passes original when non-null", () => {
    const shape = createMockShape({
      city: { nullabilityOp: { type: "withDefault", defaultValue: "Unknown" } },
    });
    const obj = createMockObject({ city: "NYC" });
    const result = applyShapeTransformations(shape, obj);
    expect(result.data).toBeDefined();
    expect((result.data as Record<string, unknown>).city).toBe("NYC");
  });
  ...
  it("empty shape returns object unchanged", () => {
    const shape = createMockShape({});
    const obj = createMockObject({ name: "Alice", age: 30 });
    const result = applyShapeTransformations(shape, obj);
    expect(result.data).toBeDefined();
    expect(result.dropped).toBe(false);
    expect(result.violations).toEqual([]);
