...
describe.each([
  ...
  it("Accepts attachments", async () => {
    ...
    await apiServer.boundary(async () => {
      apiServer.use(
        MockOntologiesV2.Actions.apply(baseUrl, (info) => {
          return {
            validation: {
              result: "VALID",
              submissionCriteria: [],
  ...
  it("Accepts media reference", async () => {
    const clientBoundActionTakesMedia = client(actionTakesMedia).applyAction;
    type InferredParamType = Parameters<typeof clientBoundActionTakesMedia>[0];
  ...
  it("Accepts geoshapes", async () => {
    const clientBoundActionTakesMedia = client(addGeoshape).applyAction;
    type InferredParamType = Parameters<typeof clientBoundActionTakesMedia>[0];
  ...
  it("Accepts interfaces", async () => {
    const clientBoundTakesInterface = client(deleteFooInterface).applyAction;
    ...
    const clientBoundBatchActionTakesInterface =
  ...
  it("Accepts object type refs", async () => {
    const clientBoundTakesObjectType = client(createFooInterface).applyAction;

    type InferredParamType = Parameters<typeof clientBoundTakesObjectType>[0];

    expectTypeOf<{
      createdInterface: string;
    }>().toMatchTypeOf<InferredParamType>();

    const clientBoundBatchActionTakesObjectType =
  ...
  it("nullable parameters work with nullish coalescing", () => {
    const clientBoundAction = client(createOffice).applyAction;

    type InferredParamType = Parameters<typeof clientBoundAction>[0];
...
describe("ActionResponse remapping", () => {
  ...
  it("Correctly unpacks edits and editedObjectTypes", () => {
    ...
    const remappedBatchActionResponse =
      remapActionResponse(batchActionResponse);
