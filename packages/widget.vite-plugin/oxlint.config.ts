...
// Nested oxlint config for @osdk/widget.vite-plugin. It inherits the whole repo ruleset by
...
export default defineConfig({
  ...
  rules: {
    ...
    "oxc/no-async-endpoint-handlers": "off", // Async request handlers are used deliberately.
    "typescript/no-non-null-assertion": "off", // `!` non-null assertions are a deliberate pattern here; not autofixable.
    "unicorn/no-object-as-default-parameter": "off", // An object literal as a default parameter; intentional.
    "unicorn/prefer-add-event-listener": "off", // `onX = ...` vs `addEventListener`; a source rewrite.
    "unicorn/prefer-import-meta-properties": "off", // `import.meta.dirname`/`filename` vs the `fileURLToPath` form; a rewrite.
    "unicorn/prefer-module": "off", // `__dirname`/`require` vs ESM meta; a source rewrite.
    "unicorn/prefer-node-protocol": "off", // The `node:` protocol on builtin imports; a source rewrite.
    "unicorn/prefer-number-properties": "off", // Global `parseInt`/`isNaN` vs `Number.*`; a source rewrite.
    "unicorn/prefer-query-selector": "off", // `getElementById` vs `querySelector`; a source rewrite.
    ...
    "unicorn/text-encoding-identifier-case": "off", // Casing of encoding strings like `utf8`; a source rewrite.
  },
});
