...
describe("SeedBuilder", () => {
  ...
  describe("link by reference", () => {
    ...
    it("stores multiple links", () => {
      const emp1 = builder.add(Employee, { employeeId: "emp-001" });
      const emp2 = builder.add(Employee, { employeeId: "emp-002" });
      const dept = builder.add(Department, { departmentId: "dept-001" });

      linkByRef(builder, "link-1", emp1, "department", dept, "employees");
      linkByRef(builder, "link-2", emp2, "department", dept, "employees");

      expect(builder.build().links).toHaveLength(2);
    });
  ...
  describe("build", () => {
    it("returns empty output for an empty builder", () => {
      const output = builder.build();
    ...
    it("includes all properties in the output objects", () => {
      ...
      const obj = builder.build().objects.Employee![0]!;
      expect(obj).toEqual({
        employeeId: "emp-001",
        firstName: "Alice",
        departmentId: "dept-001",
  ...
  describe("createSeed", () => {
    ...
    it("does not leak partial state when callback throws mid-build", () => {
      const builderRef = { current: null as SeedBuilder | null };

      expect(() =>
        createSeed((seed) => {
          builderRef.current = seed;
          seed.add(Employee, { employeeId: "emp-001" });
          seed.add(Department, { departmentId: "dept-001" });
          throw new Error("mid-build failure");
        })
      ).toThrow("mid-build failure");

      // The builder has partial state but createSeed never returns it —
      // the throw prevents build() from being called, so no SeedOutput escapes.
      // The builder instance is only reachable if someone captures it (like we did above).
      const leaked = builderRef.current!.build();
      expect(leaked.objects.Employee).toHaveLength(1);
      expect(leaked.objects.Department).toHaveLength(1);
      // This proves the builder IS mutated, but createSeed's return value
      // is never produced — the caller gets the error, not a partial output.
    });
  });
