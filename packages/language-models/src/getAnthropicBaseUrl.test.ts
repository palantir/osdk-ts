...
import { getAnthropicBaseUrl } from "./getAnthropicBaseUrl.js";
...
describe("getAnthropicBaseUrl", () => {
  ...
  it("returns the Anthropic proxy URL", () => {
    ...
    expect(getAnthropicBaseUrl({ preview: true })).toBe(
      "https://example.palantirfoundry.com/api/v1/models/anthropic",
    );
  });

  it("throws when preview is not true", () => {
    expect(() =>
      getAnthropicBaseUrl({ preview: false } as unknown as { preview: true })
    ).toThrow(
