...
/**
 * Constructs the OpenAI proxy base URL for the Foundry API Gateway.
 *
 * @param options - Must include `{ preview: true }` to acknowledge use of a beta API.
 * @returns The OpenAI proxy base URL (e.g., "https://example.palantirfoundry.com/api/v1/models/openai")
...
export function getOpenAiBaseUrl(options: PreviewOptions): string {
  ...
  return `${gatewayUrl}/api/v1/models/openai`;
}
