...
/** Which transport actually retrieved the page. */
export type FetchSource = "server" | "proxy";
...
export interface CssExtractionResult {
  ...
  /**
   * How many roles were filled from real CSS (vs synthesized). Zero signals the
   * caller to fall back to the screenshot pipeline — we require at least one
   * real color before considering CSS extraction a success.
   */
...
export async function extractTokensFromWebsite(
  ...
  // hit isn't enough to reconstruct a palette, so let the caller fall back.
