...
/**
 * Liveness check via `process.kill(pid, 0)`, which sends signal 0 which is
 * a no-op that still triggers the kernel's permission check. EPERM (we lack
 * permission) is still treated as "alive". ESRCH means dead. Works cross-
 * platform.
 */
...
/**
 * Robust loading logic because we can't import the discovery conjure API in OSS.
 */
