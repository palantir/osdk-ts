...
// Nested oxlint config for @osdk/vite-plugin-superrepo. It inherits the whole repo ruleset by
...
export default defineConfig({
  ...
  rules: {
    "oxc/no-barrel-file": "off", // The package index is an intentional re-export barrel.
    "typescript/no-dynamic-delete": "off", // `delete obj[expr]` with a computed key; intentional.
    "typescript/no-unsafe-function-type": "off", // Uses the broad `Function` type deliberately.
    "unicorn/no-useless-fallback-in-spread": "off", // `...(x || {})` fallback spread; an intentional guard.
