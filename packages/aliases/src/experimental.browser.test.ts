...
describe("experimental browser entry point", () => {
  ...
  it("shares a failed request and retries on the next read", async () => {
    const fetchImpl = mockFetch();
    vi.mocked(fetchImpl).mockRejectedValueOnce(
      new TypeError("Failed to fetch"),
    );
    vi.stubGlobal("fetch", fetchImpl);

    const reads = [Aliases.custom("apiBaseUrl"), Aliases.custom("apiBaseUrl")];
    await Promise.all(
      reads.map((read) => expect(read).rejects.toThrow("Failed to fetch")),
    );
    expect(fetchImpl).toHaveBeenCalledOnce();
