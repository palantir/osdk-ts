...
`pnpm add @osdk/react@latest`. Must install `@osdk/react` AND `@osdk/client` AND `@osdk/api` together, and the versions must line up tighter than the declared peer ranges. `@osdk/react` imports unstable APIs from `@osdk/client/unstable-do-not-use` that move between releases without deprecation.
