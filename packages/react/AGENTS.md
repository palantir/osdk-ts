...
Must install `@osdk/react` AND `@osdk/client` AND `@osdk/api` together, and versions must line up tighter than the declared peer ranges. `@osdk/react` imports its observable runtime from `@osdk/client/observable`, which is version-locked across the three packages.
