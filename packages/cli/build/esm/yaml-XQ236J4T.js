...
` ? false : t.nextContentHasIndent(e, n, r) : false;
    ...
    parse(e, n) {
      ...
      let {
        parseNode: r,
        src: s
      } = e, i = u.Node.startOfLine(s, n), o = this.items[0];
...
`:
    ...
    parse(e, n) {
      ...
      let {
        src: r
      } = e, s = r.charCodeAt(n) === 65279 ? n + 1 : n;
...
`;
    ...
    parseBlockValue(e) {
      let {
        indent: n,
        src: r
      } = this.context, s = !!this.blockIndent, i = e, o = e, a = 1;
      for (let c = r[i]; c === `
...
` && (o += a > l ? i.slice(l, a + 1) : c);
    ...
    parseCharCode(e, n, r) {
      let {
        src: s
      } = this.context, i = s.substr(e, n), a = i.length === n && /^[0-9a-fA-F]+$/.test(i) ? parseInt(i, 16) : NaN;
  ...
  var Ys = (t) => t == null || typeof t == "object" && t[Symbol.iterator]().next().done, W = class t extends V {
    ...
    addIn(e, n) {
      if (Ys(e)) this.add(n);
        ...
        else if (i === undefined && this.schema) this.set(r, xs(this.schema, s, n));
    ...
    getIn([e, ...n], r) {
      ...
      return n.length === 0 ? !r && s instanceof P ? s.value : s : s instanceof t ? s.getIn(n, r) : undefined;
    }
    hasAllNullValues() {
    ...
    setIn([e, ...n], r) {
      if (n.length === 0) this.set(e, r);
        ...
        else if (s === undefined && this.schema) this.set(e, xs(this.schema, n, r));
    ...
    toString(e, {
      ...
      let {
        ...
        indentStep: l,
        stringify: f
      } = e, m = this.type === p.Type.FLOW_MAP || this.type === p.Type.FLOW_SEQ || e.inFlow;
      m && (i += l);
...
` + this.comment.replace(/^/gm, `${c}#`), o && o()) : y && a && a(), w;
  ...
  var pe = class extends W {
    ...
    get commentBefore() {
      return this.key instanceof V ? this.key.commentBefore : undefined;
    }
    set commentBefore(e) {
...
`);
  ...
  function Wt(t, e, n, {
    ...
    for (let M; M = t[g += 1]; ) {
      ...
      if (M === `
`) n === $n && (g = Rs(t, g)), m = g + c, d = undefined;
      else {
        if (M === " " && y && y !== " " && y !== `
` && y !== "	") {
          let A = t[g + 1];
...
` && A !== "	" && (d = g);
        ...
        if (g >= m) if (d) l.push(d), m = d + c, d = undefined;
        else if (n === Ut) {
          for (; y === " " || y === "	"; ) y = M, M = t[g += 1], h = true;
          let A = g > C + 1 ? g - 2 : w - 1;
          if (f[A]) return t;
          l.push(A), f[A] = true, m = A + c, d = undefined;
        } else h = true;
      }
      y = M;
    }
...
` && n !== "	" && n !== " ") {
  ...
  function Vs(t, e) {
    for (let {
      ...
      let i = t.items[r];
      i ? (n && i.value && (i = i.value), s === undefined ? (n || !i.commentBefore) && (i.spaceBefore = true) : i.commentBefore ? i.commentBefore += `
` + s : i.commentBefore = s) : s !== undefined && (t.comment ? t.comment += `
` + s : t.comment = s);
    }
  }
  function Un(t, e) {
  ...
  function Fo(t, e) {
    let {
      handle: n,
      suffix: r
    } = e.tag, s = t.tagPrefixes.find((i) => i.handle === n);
    if (!s) {
...
`) return false;
  ...
  function Ho(t, e) {
    ...
    for (let o = 0; o < e.items.length; ++o) {
      ...
      switch (a.type) {
        ...
        case p.Type.COMMENT:
          ...
          break;
        case p.Type.MAP_KEY:
          s !== undefined && r.push(new T(s)), a.error && t.errors.push(a.error), s = me(t, a.node), i = null;
          break;
        case p.Type.MAP_VALUE:
          {
            if (s === undefined && (s = null), a.error && t.errors.push(a.error), !a.context.atLineStart && a.node && a.node.type === p.Type.MAP && !a.node.context.atLineStart) {
            ...
            Go(a, l), r.push(l), s && typeof i == "number" && a.range.start > i + 1024 && t.errors.push(Ks(e, s)), s = undefined, i = null;
          }
          break;
        default:
          s !== undefined && r.push(new T(s)), s = me(t, a), i = a.range.start, a.error && t.errors.push(a.error);
          e: for (let c = o + 1; ; ++c) {
            let l = e.items[c];
            switch (l && l.type) {
              case p.Type.BLANK_LINE:
    ...
    return s !== undefined && r.push(new T(s)), {
      comments: n,
      items: r
    };
  }
  function Xo(t, e) {
    ...
    for (let a = 0; a < e.items.length; ++a) {
      ...
      if (typeof c.char == "string") {
        ...
        if (l === ":") {
          if (s === undefined && (s = null), o === ":") {
            o = ",";
            continue;
          }
        } else if (i && (s === undefined && l !== "," && (s = null), i = false), s !== undefined && (r.push(new T(s)), s = undefined, l === ",")) {
          o = ":";
          continue;
        }
        if (l === "}") {
        ...
        before: r.length,
        comment: c.comment
      })) : s === undefined ? (o === "," && t.errors.push(new p.YAMLSemanticError(c, "Separator , missing in flow map")), s = me(t, c)) : (o !== "," && t.errors.push(new p.YAMLSemanticError(c, "Indicator : missing in flow map entry")), r.push(new T(s, me(t, c))), s = undefined, i = false);
    }
    return qs(t.errors, e), s !== undefined && r.push(new T(s)), {
      comments: n,
      items: r
    };
  }
  ...
  function ea(t, e) {
    ...
    for (let l = 0; l < e.items.length; ++l) {
      ...
      if (typeof f.char == "string") {
        ...
        if (m !== ":" && (s || i !== undefined) && (s && i === undefined && (i = a ? r.pop() : null), r.push(new T(i)), s = false, i = undefined, o = null), m === a) a = null;
        else if (!a && m === "?") s = true;
        else if (a !== "[" && m === ":" && i === undefined) {
          if (a === ",") {
...
`) {
        ...
        let m = me(t, f);
        i === undefined ? (r.push(m), c = f) : (r.push(new T(i, m)), i = undefined), o = f.range.start, a = ",";
      }
    }
    return qs(t.errors, e), i !== undefined && r.push(new T(i)), {
      comments: n,
      items: r
    };
  }
...
${e.indent}${c}` : c;
  ...
  var Xn = class t {
    ...
    setAnchor(e, n) {
      ...
      let {
        map: r
      } = this, s = e && Object.keys(r).find((i) => r[i] === e);
      if (s) {
  ...
  function va(t, e) {
    ...
    } else i.comment !== null ? (r === undefined ? n.before : n.after).push(i.comment) : i.type === Y.Type.BLANK_LINE && (s = true, r === undefined && n.before.length > 0 && !t.commentBefore && (t.commentBefore = n.before.join(`
`), n.before = []));
...
`) || null;
  ...
  var en = class t {
    ...
    get(e, n) {
      return this.contents instanceof S.Collection ? this.contents.get(e, n) : undefined;
    }
    getIn(e, n) {
      return S.isEmptyPath(e) ? !n && this.contents instanceof S.Scalar ? this.contents.value : this.contents : this.contents instanceof S.Collection ? this.contents.getIn(e, n) : undefined;
    }
    has(e) {
      return this.contents instanceof S.Collection ? this.contents.has(e) : false;
    }
    hasIn(e) {
      return S.isEmptyPath(e) ? this.contents !== undefined : this.contents instanceof S.Collection ? this.contents.hasIn(e) : false;
    }
    set(e, n) {
    ...
    toJSON(e, n) {
      let {
        ...
        mapAsMap: s,
        maxAliasCount: i
      } = this.options, o = r && (typeof e != "string" || !(this.contents instanceof S.Scalar)), a = {
        doc: this,
        indentStep: "  ",
        keep: o,
        mapAsMap: o && !!s,
    ...
    toString() {
      ...
      } else this.contents !== undefined && n.push(zt(this.contents, i));
...
var hi = ee((mi) => {
  ...
  function Ya(t, e = true, n) {
    n === undefined && typeof e == "string" && (n = e, e = true);
...
var Ie = Vi;
function G(t, e) {
  return typeof (t == null ? undefined : t.type) == "string" && (!e || e.includes(t.type));
}
function pn(t, e, n) {
...
function Z(t) {
  return Ie(t == null ? undefined : t.leadingComments);
}
function ie(t) {
  return Ie(t == null ? undefined : t.middleComments);
}
function mn(t) {
  return t == null ? undefined : t.indicatorComment;
}
function H(t) {
  return t == null ? undefined : t.trailingComment;
}
function R(t) {
  return Ie(t == null ? undefined : t.endComments);
}
function Or(t) {
  let e = [], n;
  for (let r of t.split(/( +)/)) r !== " " ? n === " " ? e.push(r) : e.push((e.pop() || "") + r) : n === undefined && e.unshift(""), n = r;
...
function Ct(t, e, n) {
  ...
  let c = x(false, r.children, -1), l = (c == null ? undefined : c.type) === "flowMappingItem" && _e(c.key) && _e(c.value);
...
function Ji(t, e, n) {
  ...
  if (a && c) return ": ";
  let l = e("key"), f = Gi(r) ? " " : "";
  if (c) return r.type === "flowMappingItem" && s.type === "flowMapping" ? l : r.type === "mappingItem" && dn(i.content, n) && !H(i.content) && ((C = s.tag) == null ? undefined : C.value) !== "tag:yaml.org,2002:set" ? [l, f, ":"] : ["? ", I(2, l)];
  let m = e("value");
  if (a) return [": ", I(2, m)];
...
function Gi(t) {
  var e;
  return ((e = t.key.content) == null ? undefined : e.type) === "alias";
}
function Hi(t) {
  if (!t) return true;
  switch (t.type) {
...
function Dr(t, e) {
  if (e.position.start.offset !== e.position.end.offset) {
    ...
    if (e.type !== "root" && e.type !== "document" && e.type !== "documentHead" && e.type !== "documentBody") {
      ...
      } = e.position, s = [r.line].concat(n.line === r.line ? [] : n.line);
      for (let i of s) {
...
function Os(t, e) {
  if (t === null || t.type === undefined && t.value === null) return null;
  switch (t.type) {
    case "ALIAS":
      return Fr(t, e);
    case "BLOCK_FOLDED":
...
//# sourceMappingURL=yaml-XQ236J4T.js.map
//# sourceMappingURL=yaml-XQ236J4T.js.map