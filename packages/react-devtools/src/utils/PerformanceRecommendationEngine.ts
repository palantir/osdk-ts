...
export class PerformanceRecommendationEngine {
  ...
  generateRecommendations(cacheSnapshot: CacheSnapshot): Recommendation[] {
    ...
    for (const wf of waterfalls) {
      const waterfallId = wf.details.childQuery
        ? `waterfall-${wf.details.parentQuery}->${wf.details.childQuery}`
        : `waterfall-${wf.details.parentQuery}`;

      if (this.dismissedRecommendations.has(waterfallId)) {
        continue;
      }
