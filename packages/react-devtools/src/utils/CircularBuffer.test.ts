...
describe("CircularBuffer", () => {
  ...
  describe("onEvict callback", () => {
    it("should call onEvict when an item is overwritten", () => {
      const evicted: number[] = [];
      const buffer = new CircularBuffer<number>(3, (item) => {
        evicted.push(item);
      });
    ...
    it("should not call onEvict when buffer is not full", () => {
      const evicted: number[] = [];
      const buffer = new CircularBuffer<number>(5, (item) => {
        evicted.push(item);
      });
