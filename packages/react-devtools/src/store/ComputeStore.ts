...
export class ComputeStore {
  ...
  getMetrics(): ComputeMetrics {
    ...
    return {
      ...
      failedCount,
      pendingCount,
      averageUsagePerRequest: fulfilledCount > 0
        ? Math.round(totalUsage / fulfilledCount)
        : 0,
      averageResponseBytes: fulfilledCount > 0
        ? Math.round(totalResponseBytes / fulfilledCount)
        : 0,
    };
  }
