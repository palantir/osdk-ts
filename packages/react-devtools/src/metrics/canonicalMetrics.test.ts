...
describe("getCanonicalMetrics", () => {
  ...
  it("computes requestsSaved and derives estimatedTimeSavedMs from it", () => {
    ...
    expect(metrics.estimatedTimeSavedMs.ready).toBe(true);
  });

  it("holds estimatedTimeSavedMs not-ready until the network baseline is ready", () => {
    // Requests saved has samples, but with too few misses the network-latency
    // baseline is not ready, so the derived estimate must stay not-ready.
    const metrics = getCanonicalMetrics(
      makeSnapshot({ cacheHits: 3, cacheMisses: 2, networkResponseTime: 200 })
    );

    expect(metrics.requestsSaved.ready).toBe(true);
    expect(metrics.avgNetworkMs.ready).toBe(false);
    expect(metrics.estimatedTimeSavedMs.ready).toBe(false);
  });
