...
// Nested oxlint config for @osdk/typescript-sdk-docs-examples. It inherits the whole repo ruleset by
...
export default defineConfig({
  ...
  rules: {
    "no-useless-catch": "off", // A try/catch that rethrows; intentional (adds a boundary).
    "import/newline-after-import": "off", // A blank line after the import block; formatting.
    "typescript/no-import-type-side-effects": "off", // The `import type` side-effect form; stylistic.
    "typescript/no-non-null-assertion": "off", // `!` non-null assertions are a deliberate pattern here; not autofixable.
    "typescript/prefer-ts-expect-error": "off", // `@ts-ignore` vs `@ts-expect-error`; a source rewrite.
