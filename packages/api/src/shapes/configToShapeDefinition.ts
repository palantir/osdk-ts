...
/**
 * Converts an inline shape config into a ShapeDefinition.
 * Produces identical __shapeId to equivalent builder usage.
 */
...
>(
  ...
  if (config.links) {
    for (const [name, linkConfig] of Object.entries(config.links)) {
      ...
      } else if (linkConfig.shape) {
        // When `linkConfig.shape` is provided without `target`, the nested
        // ShapeDefinition's __baseType will be set to the *source* type (not
        // the actual link target) and __shapeId will hash the source type
        // name instead of the target. This is safe at runtime because
        // executeDerivedLinks.ts resolves the actual target type via
        // fetchMetadata. For correct compile-time typing, users should
        // prefer `target:` with a pre-built shape.
        targetShape = configToShapeDefinition(
          baseType,
          linkConfig.shape as InlineShapeConfig<BASE>,
        );
      } else {
        // No target shape specified - create an empty shape for the base type
        targetShape = configToShapeDefinition(
          baseType,
