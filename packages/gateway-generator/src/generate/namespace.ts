...
import type {
    ...
    Response,
    ResponseType,
    StaticOperation} from "../spec";
import {
    InputType
} from "../spec";
import { sanitizeParameterName, shouldSanitizePameterName, visitTypeUnion } from "./common";
import { getJsDocs } from "./getJsDocs";
import { generateType, isOptional } from "./types";

export function generateNamespaces(namespaces: Namespace[], outputDir: string, project: Project) {
...
export function generateNamespace(namespace: Namespace, directory: Directory) {
    ...
    namespace.resources.forEach(resource => generateResource(resource, sourceFile, referenceSet));
...
export function generateResource(resource: Resource, sourceFile: SourceFile, referenceSet: Set<string>) {
...
function generateParameters(
    ...
    const pathParameterStructures: ParameterDeclarationStructure[] = pathParameters.map(parameter =>
        generateParameter(parameter, referenceSet),
    );
    ...
    if (queryParameters.length > 0) {
        transformedParameters.push({
            ...
            kind: StructureKind.Parameter,
            hasQuestionToken: queryParameters.every(param => isOptional(param.type.type)),
            type: w => {
                w.block(() => {
                    queryParameters.forEach(param => {
                        w.writeLine(
                            `${param.name}${isOptional(param.type.type) ? "?" : ""}: ${generateType(
                                param.type.type,
                                referenceSet,
                            )},`,
                        );
    ...
    if (headerParameters.length > 0) {
        transformedParameters.push({
            ...
            kind: StructureKind.Parameter,
            hasQuestionToken: headerParameters.every(param => isOptional(param.type.type)),
            type: w => {
                w.block(() => {
...
function generateParameter(parameter: Parameter, referenceSet: Set<string>): ParameterDeclarationStructure {
    return {
        kind: StructureKind.Parameter,
        name: shouldSanitizePameterName(parameter.name) ? sanitizeParameterName(parameter.name) : parameter.name,
...
function generateOperationAsFunction(
    ...
    const shouldWriteResponseContentType = response.contentType && response.contentType !== APPLICATION_JSON_CONSTANT;
    const shouldWriteRequestContentType =
        shouldWriteResponseContentType || (request.contentType && request.contentType !== APPLICATION_JSON_CONSTANT);

    return {
        kind: StructureKind.Function,
        isExported: true,
        name: operation.name,
        docs: operation.documentation ? getJsDocs(operation.documentation) : undefined,
        typeParameters: [{ name: "TResponse" }],
        returnType: `Promise<TResponse>`,
        parameters: [
            { name: "_request", kind: StructureKind.Parameter, type: `OpenApiRequest<${response.body}, TResponse>` },
            ...parameters,
        ],
        statements: writer => {
            ...
            writer.writeLine(`${operation.requestBody ? `request` : UNDEFINED_CONSTANT},`);
            if (queryParameters.length > 0) {
...
function generateRequestBody(
    ...
    return visitTypeUnion(requestType, {
        binary(_type: BinaryResponseType): { body: string; contentType: string } {
            return { body: `ReadableStream<Uint8Array> | Blob`, contentType: ANY_MEDIA_TYPE_CONSTANT };
        },
        component(type: ComponentRequestType): { body: string; contentType: string } {
...
function generateResponse(referenceSet: Set<string>, response?: Response): { body: string; contentType?: string } {
    if (!response) {
        return { body: "undefined" };
    }
    ...
    return visitTypeUnion(response.body, {
        ...
        noContent(_type: NoContentResponseBody): { body: string; contentType: string } {
            return { body: "undefined", contentType: APPLICATION_JSON_CONSTANT };
        },
        accepted(type: AcceptedResponseBody): { body: string; contentType: string } {
...
function generateResponseType(
    ...
    if (responseType.type === "binary") {
        ...
        const type = {
            ...
            contentType: `${getContentTypeFromMediaType(responseType.component.mediaType)}`,
        };
        return type;
    } else {
