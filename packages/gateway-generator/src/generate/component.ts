...
export function generateComponents(
    ...
    components.forEach(component => generateComponent(component, directory, options));
}

export function generateComponent(component: Component, directory: Directory, options: GenerateOptions) {
    ...
    switch (component.type.type) {
        ...
        case "object":
            sourceFile.addInterface({
                ...
                properties: Object.entries(component.type.object.properties).map(([propertyName, propertySpec]) => {
                    return {
                        name: propertyName,
...
function generateUnionComponent(
    ...
    Object.entries(unionType.subTypes).forEach(([subtypeKey, subtypeValue]) => {
        ...
        sourceFile.addInterface({
            isExported: true,
            name,
            extends: [generateType({ type: "reference", reference: subtypeValue }, referenceSet)],
            properties: [
                {
                    name: "type",
                    type: `"${subtypeKey}"`,
    ...
    if (options.generateVisitors) {
        sourceFile.addInterface({
            ...
            properties: [
                ...Object.entries(unionType.subTypes).map(([subTypeKey, subTypeValue]) => {
                    return {
                        name: subTypeKey,
                        type: `(obj: ${generateType(
        ...
        sourceFile.addFunction({
            ...
            statements: [
                ...Object.entries(unionType.subTypes).map(([subtypeKey]) => {
                    return `if (is${upperCaseFirstLetter(
                        subtypeKey,
