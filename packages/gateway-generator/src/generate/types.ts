...
export function generateType(dataType: DataType, referenceSet: Set<string>): string {
    return visitTypeUnion(dataType, {
        object(type: ObjectType): string {
            const properties = Object.entries(type.object.properties)
                .map(
                    ([key, value]) =>
                        `${key}${value.type.type === "optional" ? "?" : ""}: ${generateType(
                            value.type,
                            referenceSet,
                        )};`,
                )
            ...
            const subTypes = Object.entries(type.union.subTypes)
                .map(
                    ([subtypeKey, subtypeValue]) =>
                        `(` +
                        generateType({ type: "reference", reference: subtypeValue }, referenceSet) +
                        ` & { type: "${subtypeKey}" })`,
                )
                .join(" | ");
            return subTypes;
